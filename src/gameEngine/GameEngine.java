 ...
 * 
 * This class utilizes code authored and provided by Mark Baldwin and Cyndi Rader as a project skeleton.
 */
...
public class GameEngine {
	 ...
	 */
	public void interaction() { //use logic from the interaction definitions to output to player what is happening.
		for (GamePiece piece : interactingPieces) {
			...
			if (result == InteractionResult.GET_POINT) {
				player.addPoint(); 
				System.out.println("\nThe magical treasure chest has appeared in from of you. You just won a prize!\n");
			}
			...
			if (result == InteractionResult.RADIATE) {
				//warning for player of nuke
				System.out.println("\nThe Nuke radiates harmful radio waves. Be careful!\n");
			
				
				
			}
			if (result == InteractionResult.TELEPORT) {
				
				System.out.println("\nYou just teleported to another location!");
				//move player to other teleport piece.
				int teleportTarget=0;
				for(int i=0; i<interactingPieces.size(); i++) {
					GamePiece p = interactingPieces.get(i);
					
					if(p.toString()=="Portal")	{
						if(!(p.getLocation()==player.getLocation()))	{
							teleportTarget = p.getLocation();
						}
					}
				}
